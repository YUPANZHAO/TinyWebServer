    ...
    for(; m_checked_idx < m_read_idx; ++m_checked_idx) {
        ...
        if(temp == '\r') {
            if((m_checked_idx+1) == m_read_idx) {
                // 读取到\r后，读缓冲区中没数据了，状态设为LINE_OPEN，等待下一次续上
                return LINE_OPEN;
            ...
            if(m_checked_idx > 1 && m_read_buf[m_checked_idx-1] == '\r') {
                // 续上之前LINE_OPEN状态
                // 问题一: 为什么是 > 1 而不是 >= 1 ? 
                // 问题二: 若前一次分析时是因为读缓冲区中\r位于缓冲区最尾部，怎么办?
...
// 主状态机，用于分析http报文的状态
// 返回值为包含：NO_REQUEST、BAD_REQUEST、INTERNAL_ERROR
